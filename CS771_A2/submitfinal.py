...
class Tree:
	...
	def fit( self, words, verbose = False ):
		...
		my_words_idx = [ i for i in range( len( words ) ) ]
		self.root.fit( all_words = self.words, my_words_idx = my_words_idx, min_leaf_size = self.min_leaf_size, max_depth = self.max_depth, verbose = verbose )
...
class Node:
	...
	def calc_entropy_change(self, split_dict, total_words):
		entropy_change = log2(total_words)
		for split in split_dict.values():
			words_cnt = len(split)
			new_entropy = (words_cnt/total_words)*log2(words_cnt)
			entropy_change -= new_entropy
		return entropy_change

	def generate_split_dict(self, all_words, my_words_idx, query_idx):
		if query_idx == -1:
			query = ""
		else:
	...
	def process_node( self, all_words, my_words_idx, verbose ):
		...
		if self.depth == 0:
			query_idx = -1
			split_dict = self.generate_split_dict(all_words, my_words_idx, query_idx)
			return ( query_idx, split_dict )
		
		query_idx = -1
		max_entropy_change = 0
		best_split_dict = self.generate_split_dict(all_words, my_words_idx, query_idx)
		for idx in my_words_idx:
			split_dict = self.generate_split_dict(all_words, my_words_idx, idx)
			entropy_change = self.calc_entropy_change(split_dict, len(my_words_idx))
